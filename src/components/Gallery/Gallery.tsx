...
const Gallery = () => {
  ...
  return (
    <section className="buildmaster-gallery-section">
      <div className="buildmaster-gallery-container">
        <div className="buildmaster-gallery-header">
          <motion.span 
            className="buildmaster-gallery-pre-title"
          ...
          <motion.h2 
            className="buildmaster-gallery-title"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          <motion.p 
            className="buildmaster-gallery-subtitle"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          <motion.div 
            className="buildmaster-filter-tabs"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            {filters.map(filter => (
              <motion.button 
                key={filter.id}
                className={`buildmaster-filter-tab ${activeFilter === filter.id ? 'buildmaster-filter-active' : ''}`}
        ...
        {isLoading ? (
          <div className="buildmaster-loading-grid">
            {[...Array(3)].map((_, index) => (
              <div key={index} className="buildmaster-loading-card">
                <div className="buildmaster-loading-shimmer"></div>
              </div>
            ))}
          </div>
        ) : (
          <motion.div 
            className="buildmaster-gallery-grid"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          >
            <AnimatePresence>
              {filteredProjects.map((project, index) => (
                <motion.div 
                  key={project.id}
                  className="buildmaster-gallery-item"
                ...
                >
                  <div 
                    className="buildmaster-gallery-item-inner" 
                    style={{ backgroundImage: `url(${project.image})` }}
                  >
                    <div className="buildmaster-gallery-overlay">
                      <div className="buildmaster-gallery-content">
                        <div className="buildmaster-project-tags">
                          {project.tags.map(tag => (
                            <span key={tag} className="buildmaster-project-tag">{tag}</span>
                          ))}
                        </div>
                        <h3 className="buildmaster-project-title">{project.title}</h3>
                        <p className="buildmaster-project-description">{project.description}</p>
                        <motion.a 
                          href={`/projects/${project.id}`} 
                          className="buildmaster-view-project"
                          whileHover={{ x: 5 }}
                        >
        ...
        <motion.div 
          className="buildmaster-gallery-cta"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <motion.a 
            href="/projects" 
            className="buildmaster-primary-button"
            whileHover={{ 
              scale: 1.05,
          ...
          >
            ...
            <ArrowRight className="buildmaster-button-icon" size={16} />
          </motion.a>
        </motion.div>
      </div>
    </section>
